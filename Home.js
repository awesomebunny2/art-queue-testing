...
//        console.log("DOCUMENT IS LOADED Potato BABY 🚀🌕🔥")
...
Office.onReady(async (info) => {
    ...
    if (info.host === Office.HostType.Excel) {
        ...
        Excel.run(async (context) => {
            ...
            $("#nm-group").change(() => tryCatch(nmGroupToPrintLink));

            $("#normal-group").change(() => tryCatch(normalGroupToPrintLink));

            $("#print-date").change(() => tryCatch(printToGroupLink));

            $("#nm-print-date").change(() => tryCatch(nmPrintToGroupLink));

            $("#normal-print-date").change(() => tryCatch(normalPrintToGroupLink));
            ...
            $("#submit-too").on("click", () => {
                ...
                if ($("#nm-group").is(":hidden")) {
                    var newPrintDate = $("#normal-print-date").val();
                    var newGroup = $("#normal-group").val();
                };

                if ($("#normal-group").is(":hidden")) {
                    var newPrintDate = $("#nm-print-date").val();
                    var newGroup = $("#nm-group").val();
                };
...
async function newMoverGroupPrintVariation(product, nmOrNo) {
    ...
    await Excel.run(async (context) => {
        ...
        if (nmOrNo == true) {
            var printDateInput = "#nm-print-date";
            var groupInput = "#nm-group";
        } else {
            var printDateInput = "#normal-print-date";
            var groupInput = "#normal-group";
        }

        $(printDateInput).empty();
        $(printDateInput).append($("<option disabled selected hidden></option>").val("").text(""));
        $(groupInput).empty();
        ...
        if (product == "New Mover" || product == "Plastic New Mover" || product == "Birthday Postcard" || product == "New Homeowner Postcard") {
            ...
            newMoverRefValues.forEach(function (row) {
                ...
                if (xxx == 0) { // Meaning, it's not there yet, because it's length count is 0
                    ...
                    //create proper html formatting for option to be added to select box
                    var printDateOption = `<option ${printDateInput}-convert="${leDate}">${leDate}</option>`;

                    $(printDateInput).append(printDateOption);
                };
...
function nmGroupToPrintLink() {

    var group = $("#nm-group").val();
    ...
    if (group.length == 0) {
        ...
        try {
            ...
            formattedPrintDateMatch = [('' + (leNewDate.getMonth() + 1)).slice(-2), ('' + leNewDate.getDate()).slice(-2),
            (leNewDate.getFullYear() % 100)].join('/');
            $("#nm-print-date").val(formattedPrintDateMatch);
        } catch (e) {
            console.log("Error with print date autofill based on group letter input. Please debug to resolve.")
        };
...
function normalGroupToPrintLink() {

    var group = $("#normal-group").val();
    ...
    if (group.length == 0) {
        ...
        try {
            ...
            formattedPrintDateMatch = [('' + (leNewDate.getMonth() + 1)).slice(-2), ('' + leNewDate.getDate()).slice(-2),
            (leNewDate.getFullYear() % 100)].join('/');
            $("#normal-print-date").val(formattedPrintDateMatch);
        } catch (e) {
            console.log("Error with print date autofill based on group letter input. Please debug to resolve.")
        };
...
function nmPrintToGroupLink() {

    var lePrintDate = $("#nm-print-date").val();
...
function normalPrintToGroupLink() {

    var lePrintDate = $("#normal-print-date").val();
...
async function onTableChanged(eventArgs) {
    ...
    await Excel.run(async (context) => {
        ...
        var hannahTable = context.workbook.tables.getItem("HannahProjects").load("worksheet");
        var hannahTableName = context.workbook.tables.getItem("HannahProjects").load("name");
        var hannahTableRows = hannahTable.rows;
        hannahTableRows.load("items");
        var hannahRange = hannahTable.getDataBodyRange().load("values");
        var hannahHeader = hannahTable.getHeaderRowRange().load("values");
        ...
        var julieTableRows = julieTable.rows;
        julieTableRows.load("items");
        var julieRange = julieTable.getDataBodyRange().load("values");
        var julieHeader = julieTable.getHeaderRowRange().load("values");
        ...
        var alexETable = context.workbook.tables.getItem("AlexEProjects").load("worksheet");
        var alexETableName = context.workbook.tables.getItem("AlexEProjects").load("name");
        var alexETableRows = alexETable.rows;
        alexETableRows.load("items");
        var alexERange = alexETable.getDataBodyRange().load("values");
        var alexEHeader = alexETable.getHeaderRowRange().load("values");

        var alexAutoTable = context.workbook.tables.getItem("AlexAutoProjects").load("worksheet");
        var alexAutoTableName = context.workbook.tables.getItem("AlexAutoProjects").load("name");
        var alexAutoTableRows = alexAutoTable.rows;
        alexAutoTableRows.load("items");
        var alexAutoRange = alexAutoTable.getDataBodyRange().load("values");
        var alexAutoHeader = alexAutoTable.getHeaderRowRange().load("values");
        ...
        if (rowInfoConditions.includes(changedColumnIndex)
            ...
            if (changedColumnIndex == rowInfo.product.columnIndex
                ...
                showElement("#new-mover-to-normal", "hide");
                showElement("#normal-group", "hide");
                showElement("#normal-group-label", "hide");
                showElement("#normal-print-date", "hide");
                showElement("#normal-print-date-label", "hide");
                showElement("#product-group-update", "show");
                showElement("#normal-to-new-mover", "show");
                showElement("#nm-group", "show");
                showElement("#nm-group-label", "show");
                showElement("#nm-print-date", "show");
                showElement("#nm-print-date-label", "show");
            ...
            if (changedColumnIndex == rowInfo.product.columnIndex
                && (specialProducts.includes(eventArgs.details.valueBefore) // Does include
                    || (eventArgs.details.valueBefore == "Logo Recreation" && isNMGroupToo == true))
                && (!specialProducts.includes(eventArgs.details.valueAfter) // Does not include
                    || (eventArgs.details.valueAfter == "Logo Recreation" && isNMGroupToo !== true))) { //update group letter to New Mover 


                console.log("Stinky Pete");

                showElement("#normal-to-new-mover", "hide");
                showElement("#nm-group", "hide");
                showElement("#nm-group-label", "hide");
                showElement("#nm-print-date", "hide");
                showElement("#nm-print-date-label", "hide");
                showElement("#product-group-update", "show");
                showElement("#new-mover-to-normal", "show");
                showElement("#normal-group", "show");
                showElement("#normal-group-label", "show");
                showElement("#normal-print-date", "show");
                showElement("#normal-print-date-label", "show");
        ...
        if (changedColumnIndex == rowInfo.artist.columnIndex || changedColumnIndex == rowInfo.status.columnIndex) {
            ...
            if (rowInfo.artist.value == "Unassigned" && isUnassigned == false) {
            ...
            } else if (rowInfo.artist.value == "Hannah") {
                destinationTable = hannahTable;
                destinationTableName = hannahTableName.name;
                destinationRows = hannahTableRows.items;
                destinationTableRange = hannahRange;
                destinationHeader = hannahHeader;
                ...
                destinationRows = julieTableRows.items;
                destinationTableRange = julieRange;
                destinationHeader = julieHeader;
            ...
            } else if (rowInfo.artist.value == "Alex E.") {
                destinationTable = alexETable;
                destinationTableName = alexETableName.name;
                destinationRows = alexETableRows.items;
                destinationTableRange = alexERange;
                destinationHeader = alexEHeader;
            } else if (rowInfo.artist.value == "Alex Auto") {
                destinationTable = alexAutoTable;
                destinationTableName = alexAutoTableName.name;
                destinationRows = alexAutoTableRows.items;
                destinationTableRange = alexAutoRange;
                destinationHeader = alexAutoHeader;
                ...
                if (
                    ...
                    if (destinationTable !== "null") { //as long as there is a destination value, do the following
                        ...
                        if (rowInfo.artist.value == "Unassigned" && isUnassigned == false) {
                            ...
                            var destinationStation = hannahRange;
                        } else if (rowInfo.artist.value == "Luke") {
                            ...
                            var destinationStation = alexERange;
                        } else if (rowInfo.artist.value == "Alex Auto") {
                            var destinationStation = alexAutoRange;
                        } else if (rowInfo.artist.value == "Jamie") {
